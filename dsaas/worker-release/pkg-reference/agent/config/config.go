...
func get(ctx context.Context, envInfo *cmds.Agent, proxy proxy.Proxy) (*config.Node, error) {
	...
	info, err := parseAndValidateTokenWithDaaS(ctx, proxy.SupervisorURL(), envInfo.Token, withCert, envInfo)
	if err != nil {
		return nil, err
	}
...
// parseAndValidateTokenWithDaaS handles both traditional and DaaS authentication
func parseAndValidateTokenWithDaaS(ctx context.Context, serverURL, token string, options ...clientaccess.ClientAccessOption) (*clientaccess.Info, error) {
	// Check if this is a Seal token
	if security.IsSealToken(token) {
		return parseAndValidateSealToken(ctx, serverURL, token, options...)
	}

	// Fall back to traditional token validation for backward compatibility
	return clientaccess.ParseAndValidateToken(serverURL, token, options...)
}

// parseAndValidateSealToken handles Seal-based authentication
func parseAndValidateSealToken(ctx context.Context, serverURL, token string, options ...clientaccess.ClientAccessOption) (*clientaccess.Info, error) {
	...
	if suiClient != nil {
		minStakeStr := daasConfig.StakeConfig.MinStake
		minStake, _ := strconv.ParseUint(minStakeStr, 10, 64)
	...
	// Create client access info
	// For POC, we'll create a minimal info object
	// In production, this would involve proper server communication
	info := &clientaccess.Info{
		BaseURL:    serverURL,
		CACerts:    []byte{}, // Would be populated from server
		ClientCert: []byte{}, // Would be generated
		ClientKey:  []byte{}, // Would be generated
	}
