...
func createProxyAndValidateToken(ctx context.Context, cfg *cmds.Agent) (proxy.Proxy, error) {
	...
	// Initialize DaaS configuration
	daasConfig := security.DefaultDaaSConfig()
	daasValidator, err := security.NewDaaSValidator(daasConfig)
	if err != nil {
		logrus.Warnf("Failed to initialize DaaS validator, falling back to traditional auth: %v", err)
	}
	...
	for {
		var newToken *clientaccess.Info
		var err error

		// Try DaaS validation if available and token looks like Seal token
		if daasValidator != nil && daasValidator.IsEnabled() && security.IsSealToken(cfg.Token) {
			logrus.Info("Attempting DaaS authentication with Seal token")
			newToken, err = validateSealTokenWithDaaS(ctx, proxy.SupervisorURL(), cfg.Token, options, daasValidator)
		} else {
			// Fall back to traditional token validation
...
// validateSealTokenWithDaaS validates a Seal token using DaaS authentication
func validateSealTokenWithDaaS(ctx context.Context, serverURL, token string, options []clientaccess.ValidationOption, validator *security.DaaSValidator) (*clientaccess.Info, error) {
	// Parse Seal token
	...
	// Validate stake using Sui client
	suiClient := validator.GetSuiClient()
	if suiClient != nil {
		// Parse minimum stake from config
		minStake := uint64(1000000000) // 1 SUI default
		...
		logrus.Infof("DaaS: Worker stake validated - Address: %s, Stake: %d", stakeInfo.WalletAddress, stakeInfo.StakeAmount)
	}

	// Validate Seal signature
	sealAuth := validator.GetSealAuth()
	if sealAuth != nil {
		if err := sealAuth.ValidateToken(sealToken); err != nil {
			return nil, fmt.Errorf("Seal signature validation failed: %w", err)
		}
		logrus.Info("DaaS: Seal signature validated successfully")
	}

	// Create client access info for successful DaaS authentication
	info := &clientaccess.Info{
		URL:      serverURL,
		Token:    token,
		Username: "system:node:" + sealToken.WalletAddress, // Use wallet address as node name
		Password: sealToken.Signature,                      // Use signature as password
	}

	logrus.Infof("DaaS authentication successful for wallet: %s", sealToken.WalletAddress)
	return info, nil
}
